...
const decodeJwtPayload = (jwt) => {
    ...
    if (parts.length !== 3) {
        throw new NativeAuthError('Invalid JWT structure returned from native auth endpoint', {
            code: 'invalid_jwt'
        });
    }
...
export const normalizeNativeAuthError = (error, correlationId, context) => {
    if (!(error instanceof NativeAuthError)) {
        context?.log?.error?.('[NativeAuth] Unexpected error type', safeStringify({
            correlationId,
    ...
    if (code === 'redirect') {
        ...
        httpStatus = 401;
        message = 'The authentication session expired. Restart the flow.';
...
export const normalizeNativeSignUpError = (error, correlationId, context) => {
    if (!(error instanceof NativeAuthError)) {
        context?.log?.error?.('[NativeAuth][SignUp] Unexpected error type', safeStringify({
            correlationId,
            message: error?.message
        }));
        return {
...
export const passwordSignInFlow = async ({ config, email, password, correlationId, context }) => {
    const challengeTypeString = config.passwordChallengeString || config.passwordChallenges?.join(' ') || config.challengeTypeString;
    ...
    if (initiateData?.challenge_type === 'redirect') {
        throw new NativeAuthError('Redirect required for native auth sign-in', {
            status: 400,
            code: 'redirect'
        });
    }
    ...
    if (!challengeType || challengeType !== 'password') {
        ...
        if (challengeType && challengeType !== 'password') {
            throw new NativeAuthError(`Unsupported authentication challenge type: ${challengeType}`, {
                status: 400,
                code: challengeType
            });
        }
    ...
    return {
        ...
        scope: tokenData.scope || joinScopes(config.scopes),
...
export const signUpStartFlow = async ({ config, email, password, attributesPayload, challengeTypeString, correlationId, context }) => {
    const params = {
        client_id: config.clientId,
        username: email,
        password,
        challenge_type: challengeTypeString || config.signupChallengeTypeString || config.challengeTypeString,
        channel_hint: 'email'
    };

    if (attributesPayload) {
        params.attributes = attributesPayload;
    }
...
export const passwordResetStartFlow = async ({ config, username, correlationId, context }) => {
    const challengeTypeString = config.signupChallengeTypeString || config.challengeTypeString;
