...
const authorizeRequest = async (req, context, options = {}) => {
...
const buildAuthFailureResponse = (error, correlationId) => {
    const status = error instanceof AuthorizationError ? error.statusCode : 401;
    const sanitizedMessage = status === 401 ? 'Authentication is required to access this resource.' : 'You do not have permission to perform this action.';
